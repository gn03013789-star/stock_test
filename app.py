...
st.caption("選擇市場與標的，線上即時預覽分析報告。")

# --------------------------------------------------------------------------- #
# 查詢區（市場切換即時連動；查詢方式二擇一，避免清單與輸入同時並行）
# --------------------------------------------------------------------------- #
market_label = st.radio("市場", ["台股", "美股"], horizontal=True,
                        key="market_label")
# 切換市場時清掉上一檔的殘留結果
...
# 查詢方式二擇一：避免下拉與輸入同時出現（輸入框才會叫出鍵盤）
mode = st.radio("查詢方式", ["📋 從清單選取", "⌨️ 自行輸入"], horizontal=True,
                key="query_mode")

query_pick = ""
if mode == "📋 從清單選取":
    options = [f"{sid}　{name}" for sid, name in universe]
    picked = st.selectbox(f"{market_label}市值前百大（約略排序快照）", options,
                          key=f"picked_{market}")
    query_pick = picked.split("　")[0].strip() if picked else ""
else:
    manual = st.text_input("輸入股票代號／名稱",
                           placeholder="例如：2330、台積電、AAPL",
                           key="manual_input")
    query_pick = manual.strip()

submitted = st.button("產生報告", type="primary", use_container_width=True)

query = ""
if submitted:
    st.session_state.pop("report", None)  # 重新搜尋先清掉殘留
    if query_pick:
        query = query_pick
    else:
        st.warning("請選取一檔，或輸入代號／名稱。")
...
def render_preview(report):
    ...
    # 若使用者在「預測」分頁改了預測期間，這裡重算預測（僅用既有股價資料，不重抓網路）。
    ...
    if sel == L_PRICE:
        ...
        if report.price.points:
            ...
            # 均線多空排列判斷
            if report.technical is not None and report.technical.ma_alignment:
                st.caption(f"📐 {report.technical.ma_alignment}")
        else:
...
if "report" in st.session_state:
    render_preview(st.session_state["report"])
